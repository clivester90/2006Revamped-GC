...
final class SoundFilter {
	...
	public void method545(ByteBuffer byteBuffer, SoundEnvelope soundEnvelope) {
		int i = byteBuffer.readUnsignedByte();
		anIntArray665[0] = i >> 4;
		anIntArray665[1] = i & 0xf;
		if (i != 0) {
			anIntArray668[0] = byteBuffer.readUnsignedWord();
			anIntArray668[1] = byteBuffer.readUnsignedWord();
			int j = byteBuffer.readUnsignedByte();
			for (int k = 0; k < 2; k++) {
				for (int l = 0; l < anIntArray665[k]; l++) {
					anIntArrayArrayArray666[k][0][l] = byteBuffer.readUnsignedWord();
					anIntArrayArrayArray667[k][0][l] = byteBuffer.readUnsignedWord();
				}
			...
			for (int i1 = 0; i1 < 2; i1++) {
				for (int j1 = 0; j1 < anIntArray665[i1]; j1++) {
					if ((j & 1 << i1 * 4 << j1) != 0) {
						anIntArrayArrayArray666[i1][1][j1] = byteBuffer.readUnsignedWord();
						anIntArrayArrayArray667[i1][1][j1] = byteBuffer.readUnsignedWord();
					} else {
			...
			if (j != 0 || anIntArray668[1] != anIntArray668[0]) {
				soundEnvelope.method326(byteBuffer);
			}
		} else {
