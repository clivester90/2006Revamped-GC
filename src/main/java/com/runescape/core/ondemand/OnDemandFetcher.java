...
import com.runescape.core.node.NodeSubList;
...
public final class OnDemandFetcher extends OnDemandFetcherParent implements Runnable {
	...
	public void start(StreamLoader streamLoader, Game client1) {
		...
		for (int i = 0; i < 4; i++) {
			...
			for (int l = 0; l < j; l++) {
				versions[i][l] = byteBuffer.readUnsignedWord();
			}
		...
		for (int k = 0; k < 4; k++) {
			...
			ByteBuffer byteBuffer_1 = new ByteBuffer(abyte1);
			crcs[k] = new int[i1];
			for (int l1 = 0; l1 < i1; l1++) {
				crcs[k][l1] = byteBuffer_1.readDWord();
			}
		...
		ByteBuffer byteBuffer2 = new ByteBuffer(abyte2);
		j1 = abyte2.length / 7;
		...
		for (int i2 = 0; i2 < j1; i2++) {
			mapIndices1[i2] = byteBuffer2.readUnsignedWord();
			mapIndices2[i2] = byteBuffer2.readUnsignedWord();
			mapIndices3[i2] = byteBuffer2.readUnsignedWord();
			mapIndices4[i2] = byteBuffer2.readUnsignedByte();
		}

		abyte2 = streamLoader.getDataForName("anim_index");
		byteBuffer2 = new ByteBuffer(abyte2);
		j1 = abyte2.length / 2;
		anIntArray1360 = new int[j1];
		for (int j2 = 0; j2 < j1; j2++) {
			anIntArray1360[j2] = byteBuffer2.readUnsignedWord();
		}
		...
		for (int k2 = 0; k2 < j1; k2++) {
			anIntArray1348[k2] = byteBuffer2.readUnsignedByte();
		}
