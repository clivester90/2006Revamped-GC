...
public final class Censor {
	...
	public static void loadConfig(StreamLoader streamLoader) {
		ByteBuffer byteBuffer = new ByteBuffer(streamLoader.getDataForName("fragmentsenc.txt"));
		ByteBuffer byteBuffer_1 = new ByteBuffer(streamLoader.getDataForName("badenc.txt"));
		ByteBuffer byteBuffer_2 = new ByteBuffer(streamLoader.getDataForName("domainenc.txt"));
		ByteBuffer byteBuffer_3 = new ByteBuffer(streamLoader.getDataForName("tldlist.txt"));
		readValues(byteBuffer, byteBuffer_1, byteBuffer_2, byteBuffer_3);
	}

	private static void readValues(ByteBuffer byteBuffer, ByteBuffer byteBuffer_1, ByteBuffer byteBuffer_2, ByteBuffer byteBuffer_3) {
		readBadEnc(byteBuffer_1);
		readDomainEnc(byteBuffer_2);
		readFragmentsEnc(byteBuffer);
		readTldList(byteBuffer_3);
	}

	private static void readTldList(ByteBuffer byteBuffer) {
		int i = byteBuffer.readDWord();
		aCharArrayArray624 = new char[i][];
		anIntArray625 = new int[i];
		for (int j = 0; j < i; j++) {
			anIntArray625[j] = byteBuffer.readUnsignedByte();
			char[] ac = new char[byteBuffer.readUnsignedByte()];
			for (int k = 0; k < ac.length; k++) {
				ac[k] = (char) byteBuffer.readUnsignedByte();
			}
	...
	private static void readBadEnc(ByteBuffer byteBuffer) {
		int j = byteBuffer.readDWord();
		aCharArrayArray621 = new char[j][];
		aByteArrayArrayArray622 = new byte[j][][];
		method493(byteBuffer, aCharArrayArray621, aByteArrayArrayArray622);
	}

	private static void readDomainEnc(ByteBuffer byteBuffer) {
		int i = byteBuffer.readDWord();
		aCharArrayArray623 = new char[i][];
		method494(aCharArrayArray623, byteBuffer);
	}

	private static void readFragmentsEnc(ByteBuffer byteBuffer) {
		anIntArray620 = new int[byteBuffer.readDWord()];
		for (int i = 0; i < anIntArray620.length; i++) {
			anIntArray620[i] = byteBuffer.readUnsignedWord();
		}
	}

	private static void method493(ByteBuffer byteBuffer, char[][] ac, byte[][][] abyte0) {
		for (int j = 0; j < ac.length; j++) {
			char[] ac1 = new char[byteBuffer.readUnsignedByte()];
			for (int k = 0; k < ac1.length; k++) {
				ac1[k] = (char) byteBuffer.readUnsignedByte();
			}

			ac[j] = ac1;
			byte[][] abyte1 = new byte[byteBuffer.readUnsignedByte()][2];
			for (int l = 0; l < abyte1.length; l++) {
				abyte1[l][0] = (byte) byteBuffer.readUnsignedByte();
				abyte1[l][1] = (byte) byteBuffer.readUnsignedByte();
			}
	...
	private static void method494(char[][] ac, ByteBuffer byteBuffer) {
		for (int j = 0; j < ac.length; j++) {
			char[] ac1 = new char[byteBuffer.readUnsignedByte()];
			for (int k = 0; k < ac1.length; k++) {
				ac1[k] = (char) byteBuffer.readUnsignedByte();
