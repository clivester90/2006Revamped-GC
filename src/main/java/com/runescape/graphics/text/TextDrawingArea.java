...
public class TextDrawingArea extends DrawingArea {
	...
	public TextDrawingArea(boolean flag, String s, StreamLoader streamLoader) {
		...
		byteBuffer_1.currentOffset = byteBuffer.readUnsignedWord() + 4;
		int k = byteBuffer_1.readUnsignedByte();
		if (k > 0) {
			byteBuffer_1.currentOffset += 3 * (k - 1);
		}
		for (int l = 0; l < 256; l++) {
			anIntArray1494[l] = byteBuffer_1.readUnsignedByte();
			anIntArray1495[l] = byteBuffer_1.readUnsignedByte();
			int i1 = anIntArray1492[l] = byteBuffer_1.readUnsignedWord();
			int j1 = anIntArray1493[l] = byteBuffer_1.readUnsignedWord();
			int k1 = byteBuffer_1.readUnsignedByte();
			int l1 = i1 * j1;
			aByteArrayArray1491[l] = new byte[l1];
			if (k1 == 0) {
				for (int i2 = 0; i2 < l1; i2++) {
					aByteArrayArray1491[l][i2] = byteBuffer.readSignedByte();
				}

			} else if (k1 == 1) {
				for (int j2 = 0; j2 < i1; j2++) {
					for (int l2 = 0; l2 < j1; l2++) {
						aByteArrayArray1491[l][j2 + l2 * i1] = byteBuffer.readSignedByte();
					}
