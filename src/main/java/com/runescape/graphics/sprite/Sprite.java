package com.runescape.graphics.sprite;// Decompiled by Jad v1.5.8f. Copyright 2001 Pavel Kouznetsov.
// Jad home page: http://www.kpdus.com/jad.html
// Decompiler options: packimports(3) 

import com.runescape.graphics.Background;
...
public class Sprite extends DrawingArea {
	...
	public Sprite(StreamLoader streamLoader, String s, int i) {
		ByteBuffer byteBuffer = new ByteBuffer(streamLoader.getDataForName(s + ".dat"));
		ByteBuffer byteBuffer_1 = new ByteBuffer(streamLoader.getDataForName("index.dat"));
		byteBuffer_1.currentOffset = byteBuffer.readUnsignedWord();
		maxWidth = byteBuffer_1.readUnsignedWord();
		maxHeight = byteBuffer_1.readUnsignedWord();
		int j = byteBuffer_1.readUnsignedByte();
		int[] ai = new int[j];
		for (int k = 0; k < j - 1; k++) {
			ai[k + 1] = byteBuffer_1.read3Bytes();
			if (ai[k + 1] == 0) {
				ai[k + 1] = 1;
			}
		}
		...
		for (int l = 0; l < i; l++) {
			byteBuffer_1.currentOffset += 2;
			byteBuffer.currentOffset += byteBuffer_1.readUnsignedWord() * byteBuffer_1.readUnsignedWord();
			byteBuffer_1.currentOffset++;
		}

		spriteOffsetX = byteBuffer_1.readUnsignedByte();
		spriteOffsetY = byteBuffer_1.readUnsignedByte();
		myWidth = byteBuffer_1.readUnsignedWord();
		height = byteBuffer_1.readUnsignedWord();
		int i1 = byteBuffer_1.readUnsignedByte();
		int j1 = myWidth * height;
		pixels = new int[j1];
		if (i1 == 0) {
			for (int k1 = 0; k1 < j1; k1++) {
				pixels[k1] = ai[byteBuffer.readUnsignedByte()];
			}
		...
		if (i1 == 1) {
			for (int l1 = 0; l1 < myWidth; l1++) {
				for (int i2 = 0; i2 < height; i2++) {
					pixels[l1 + i2 * myWidth] = ai[byteBuffer.readUnsignedByte()];
				}
	...
	public void method354(Background background, int i, int j) {
		...
		if (!(j1 <= 0 || i1 <= 0)) {
			method355(pixels, j1, background.data, i1, DrawingArea.pixels, 0, k1, k, l1, l);
		}
	}
