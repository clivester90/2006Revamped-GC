...
public class Texture extends DrawingArea {
	...
	public static void prepare3d2(int j, int k) {
	...
	public static void setupTextures() {
		if (texturePool == null) {
			...
			if (lowMem) {
				texturePool = new int[texturePosition][16384];
			} else {
				texturePool = new int[texturePosition][0x10000];
			}
			for (int k = 0; k < 50; k++) {
	...
	private static int[] getTexturePixels(int textureId) {
		...
		if (texturePosition > 0) {
			ai = texturePool[--texturePosition];
			texturePool[texturePosition] = null;
		} else {
			int j = 0;
			int k = -1;
			for (int l = 0; l < textureCount; l++) {
				if (texturePixels[l] != null && (textureCycle[l] < j || k == -1)) {
					j = textureCycle[l];
					k = l;
				}
			}
			...
			if (background.myWidth == 64) {
				for (int j1 = 0; j1 < 128; j1++) {
					for (int j2 = 0; j2 < 128; j2++) {
						ai[j2 + (j1 << 7)] = ai1[background.data[(j2 >> 1) + (j1 >> 1 << 6)]];
					}
	...
	public static void drawNewMaterializedTriangle(int i, int j, int k, int l, int i1, int j1, int k1) {
		int l1 = 0;
		if (j != i) {
			l1 = (i1 - l << 16) / (j - i);
		}
		...
		if (i <= j && i <= k) {
			...
			if (j < k) {
				...
				if (i != j && j2 < l1 || i == j && j2 > i2) {
					...
					j -= i;
					for (i = lineOffsets[i]; --j >= 0; i += DrawingArea.width) {
						drawNewMaterializedScanline(DrawingArea.pixels, i, k1, j1 >> 16, l >> 16);
						j1 += j2;
						l += l1;
					}

					while (--k >= 0) {
						drawNewMaterializedScanline(DrawingArea.pixels, i, k1, j1 >> 16, i1 >> 16);
						j1 += j2;
						i1 += i2;
						i += DrawingArea.width;
					}
				...
				j -= i;
				for (i = lineOffsets[i]; --j >= 0; i += DrawingArea.width) {
					drawNewMaterializedScanline(DrawingArea.pixels, i, k1, l >> 16, j1 >> 16);
					j1 += j2;
					l += l1;
				}

				while (--k >= 0) {
					drawNewMaterializedScanline(DrawingArea.pixels, i, k1, i1 >> 16, j1 >> 16);
					j1 += j2;
					i1 += i2;
					i += DrawingArea.width;
				}
			...
			if (i != k && j2 < l1 || i == k && i2 > l1) {
				...
				k -= i;
				for (i = lineOffsets[i]; --k >= 0; i += DrawingArea.width) {
					drawNewMaterializedScanline(DrawingArea.pixels, i, k1, i1 >> 16, l >> 16);
					i1 += j2;
					l += l1;
				}
			...
			k -= i;
			for (i = lineOffsets[i]; --k >= 0; i += DrawingArea.width) {
				drawNewMaterializedScanline(DrawingArea.pixels, i, k1, l >> 16, i1 >> 16);
				i1 += j2;
				l += l1;
			}
		...
		if (j <= k) {
			...
			if (k < i) {
				...
				if (j != k && l1 < i2 || j == k && l1 > j2) {
					...
					k -= j;
					for (j = lineOffsets[j]; --k >= 0; j += DrawingArea.width) {
						drawNewMaterializedScanline(DrawingArea.pixels, j, k1, l >> 16, i1 >> 16);
						l += l1;
						i1 += i2;
					}

					while (--i >= 0) {
						drawNewMaterializedScanline(DrawingArea.pixels, j, k1, l >> 16, j1 >> 16);
						l += l1;
						j1 += j2;
						j += DrawingArea.width;
					}
				...
				k -= j;
				for (j = lineOffsets[j]; --k >= 0; j += DrawingArea.width) {
					drawNewMaterializedScanline(DrawingArea.pixels, j, k1, i1 >> 16, l >> 16);
					l += l1;
					i1 += i2;
				}

				while (--i >= 0) {
					drawNewMaterializedScanline(DrawingArea.pixels, j, k1, j1 >> 16, l >> 16);
					l += l1;
					j1 += j2;
					j += DrawingArea.width;
				}
			...
			if (l1 < i2) {
				...
				i -= j;
				for (j = lineOffsets[j]; --i >= 0; j += DrawingArea.width) {
					drawNewMaterializedScanline(DrawingArea.pixels, j, k1, j1 >> 16, i1 >> 16);
					j1 += l1;
					i1 += i2;
				}
			...
			i -= j;
			for (j = lineOffsets[j]; --i >= 0; j += DrawingArea.width) {
				drawNewMaterializedScanline(DrawingArea.pixels, j, k1, i1 >> 16, j1 >> 16);
				j1 += l1;
				i1 += i2;
			}
		...
		if (i < j) {
			...
			if (i2 < j2) {
				...
				i -= k;
				for (k = lineOffsets[k]; --i >= 0; k += DrawingArea.width) {
					drawNewMaterializedScanline(DrawingArea.pixels, k, k1, i1 >> 16, j1 >> 16);
					i1 += i2;
					j1 += j2;
				}

				while (--j >= 0) {
					drawNewMaterializedScanline(DrawingArea.pixels, k, k1, i1 >> 16, l >> 16);
					i1 += i2;
					l += l1;
					k += DrawingArea.width;
				}
			...
			i -= k;
			for (k = lineOffsets[k]; --i >= 0; k += DrawingArea.width) {
				drawNewMaterializedScanline(DrawingArea.pixels, k, k1, j1 >> 16, i1 >> 16);
				i1 += i2;
				j1 += j2;
			}

			while (--j >= 0) {
				drawNewMaterializedScanline(DrawingArea.pixels, k, k1, l >> 16, i1 >> 16);
				i1 += i2;
				l += l1;
				k += DrawingArea.width;
			}
		...
		if (i2 < j2) {
			...
			j -= k;
			for (k = lineOffsets[k]; --j >= 0; k += DrawingArea.width) {
				drawNewMaterializedScanline(DrawingArea.pixels, k, k1, l >> 16, j1 >> 16);
				l += i2;
				j1 += j2;
			}
		...
		j -= k;
		for (k = lineOffsets[k]; --j >= 0; k += DrawingArea.width) {
			drawNewMaterializedScanline(DrawingArea.pixels, k, k1, j1 >> 16, l >> 16);
			l += i2;
			j1 += j2;
		}
	...
	private static void drawNewMaterializedScanline(int[] ai, int i, int j, int l, int i1) {
		int k;// was parameter
		if (checkBounds) {
			if (i1 > DrawingArea.centerX) {
				i1 = DrawingArea.centerX;
	...
	public static void drawMaterializedTriangle(int i, int j, int k, int l, int i1, int j1, int k1, int l1, int i2, int j2, int k2, int l2, int i3, int j3, int k3, int l3, int i4, int j4, int k4) {
		int[] ai = getTexturePixels(k4);
		aBoolean1463 = !textureIsTranslucent[k4];
		k2 = j2 - k2;
		j3 = i3 - j3;
		i4 = l3 - i4;
		l2 -= j2;
		...
		if (i <= j && i <= k) {
			...
			if (j < k) {
				...
				if (i != j && i8 < i7 || i == j && i8 > k7) {
					...
					i = lineOffsets[i];
					while (--j >= 0) {
						drawMaterializedScanline(DrawingArea.pixels, ai, i, j1 >> 16, l >> 16, i2 >> 8, k1 >> 8, l4, k5, j6, i5, l5, k6);
						j1 += i8;
						l += i7;
						i2 += j8;
						k1 += j7;
					...
					while (--k >= 0) {
						drawMaterializedScanline(DrawingArea.pixels, ai, i, j1 >> 16, i1 >> 16, i2 >> 8, l1 >> 8, l4, k5, j6, i5, l5, k6);
						j1 += i8;
						i1 += k7;
						i2 += j8;
						l1 += l7;
				...
				i = lineOffsets[i];
				while (--j >= 0) {
					drawMaterializedScanline(DrawingArea.pixels, ai, i, l >> 16, j1 >> 16, k1 >> 8, i2 >> 8, l4, k5, j6, i5, l5, k6);
					j1 += i8;
					l += i7;
					i2 += j8;
					k1 += j7;
				...
				while (--k >= 0) {
					drawMaterializedScanline(DrawingArea.pixels, ai, i, i1 >> 16, j1 >> 16, l1 >> 8, i2 >> 8, l4, k5, j6, i5, l5, k6);
					j1 += i8;
					i1 += k7;
					i2 += j8;
					l1 += l7;
			...
			if (i != k && i8 < i7 || i == k && k7 > i7) {
				...
				i = lineOffsets[i];
				while (--k >= 0) {
					drawMaterializedScanline(DrawingArea.pixels, ai, i, i1 >> 16, l >> 16, l1 >> 8, k1 >> 8, l4, k5, j6, i5, l5, k6);
					i1 += i8;
					l += i7;
					l1 += j8;
					k1 += j7;
			...
			i = lineOffsets[i];
			while (--k >= 0) {
				drawMaterializedScanline(DrawingArea.pixels, ai, i, l >> 16, i1 >> 16, k1 >> 8, l1 >> 8, l4, k5, j6, i5, l5, k6);
				i1 += i8;
				l += i7;
				l1 += j8;
				k1 += j7;
		...
		if (j <= k) {
			...
			if (k < i) {
				...
				if (j != k && i7 < k7 || j == k && i7 > i8) {
					...
					j = lineOffsets[j];
					while (--k >= 0) {
						drawMaterializedScanline(DrawingArea.pixels, ai, j, l >> 16, i1 >> 16, k1 >> 8, l1 >> 8, l4, k5, j6, i5, l5, k6);
						l += i7;
						i1 += k7;
						k1 += j7;
						l1 += l7;
					...
					while (--i >= 0) {
						drawMaterializedScanline(DrawingArea.pixels, ai, j, l >> 16, j1 >> 16, k1 >> 8, i2 >> 8, l4, k5, j6, i5, l5, k6);
						l += i7;
						j1 += i8;
						k1 += j7;
						i2 += j8;
				...
				j = lineOffsets[j];
				while (--k >= 0) {
					drawMaterializedScanline(DrawingArea.pixels, ai, j, i1 >> 16, l >> 16, l1 >> 8, k1 >> 8, l4, k5, j6, i5, l5, k6);
					l += i7;
					i1 += k7;
					k1 += j7;
					l1 += l7;
				...
				while (--i >= 0) {
					drawMaterializedScanline(DrawingArea.pixels, ai, j, j1 >> 16, l >> 16, i2 >> 8, k1 >> 8, l4, k5, j6, i5, l5, k6);
					l += i7;
					j1 += i8;
					k1 += j7;
					i2 += j8;
			...
			if (i7 < k7) {
				...
				j = lineOffsets[j];
				while (--i >= 0) {
					drawMaterializedScanline(DrawingArea.pixels, ai, j, j1 >> 16, i1 >> 16, i2 >> 8, l1 >> 8, l4, k5, j6, i5, l5, k6);
					j1 += i7;
					i1 += k7;
					i2 += j7;
					l1 += l7;
			...
			j = lineOffsets[j];
			while (--i >= 0) {
				drawMaterializedScanline(DrawingArea.pixels, ai, j, i1 >> 16, j1 >> 16, l1 >> 8, i2 >> 8, l4, k5, j6, i5, l5, k6);
				j1 += i7;
				i1 += k7;
				i2 += j7;
				l1 += l7;
		...
		if (i < j) {
			...
			if (k7 < i8) {
				...
				k = lineOffsets[k];
				while (--i >= 0) {
					drawMaterializedScanline(DrawingArea.pixels, ai, k, i1 >> 16, j1 >> 16, l1 >> 8, i2 >> 8, l4, k5, j6, i5, l5, k6);
					i1 += k7;
					j1 += i8;
					l1 += l7;
					i2 += j8;
				...
				while (--j >= 0) {
					drawMaterializedScanline(DrawingArea.pixels, ai, k, i1 >> 16, l >> 16, l1 >> 8, k1 >> 8, l4, k5, j6, i5, l5, k6);
					i1 += k7;
					l += i7;
					l1 += l7;
					k1 += j7;
			...
			k = lineOffsets[k];
			while (--i >= 0) {
				drawMaterializedScanline(DrawingArea.pixels, ai, k, j1 >> 16, i1 >> 16, i2 >> 8, l1 >> 8, l4, k5, j6, i5, l5, k6);
				i1 += k7;
				j1 += i8;
				l1 += l7;
				i2 += j8;
			...
			while (--j >= 0) {
				drawMaterializedScanline(DrawingArea.pixels, ai, k, l >> 16, i1 >> 16, k1 >> 8, l1 >> 8, l4, k5, j6, i5, l5, k6);
				i1 += k7;
				l += i7;
				l1 += l7;
				k1 += j7;
		...
		if (k7 < i8) {
			...
			k = lineOffsets[k];
			while (--j >= 0) {
				drawMaterializedScanline(DrawingArea.pixels, ai, k, l >> 16, j1 >> 16, k1 >> 8, i2 >> 8, l4, k5, j6, i5, l5, k6);
				l += k7;
				j1 += i8;
				k1 += l7;
				i2 += j8;
		...
		k = lineOffsets[k];
		while (--j >= 0) {
			drawMaterializedScanline(DrawingArea.pixels, ai, k, j1 >> 16, l >> 16, i2 >> 8, k1 >> 8, l4, k5, j6, i5, l5, k6);
			l += k7;
			j1 += i8;
			k1 += l7;
			i2 += j8;
	...
	private static void drawMaterializedScanline(int[] ai, int[] ai1, int k, int l, int i1, int j1, int k1, int l1, int i2, int j2, int k2, int l2, int i3) {
	...
	private static boolean[] textureIsTranslucent = new boolean[50];
	private static int[] textureRGB = new int[50];
	...
	static {
		...
		for (int k = 0; k < 2048; k++) {
			sin[k] = (int) (65536D * Math.sin(k * 0.0030679614999999999D));
			cos[k] = (int) (65536D * Math.cos(k * 0.0030679614999999999D));
		}
