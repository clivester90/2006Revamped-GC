...
public class ItemDef {
	...
	public static void unpackConfig(StreamLoader streamLoader) {
		ItemDef.byteBuffer = new ByteBuffer(streamLoader.getDataForName("obj.dat"));
		ByteBuffer byteBuffer = new ByteBuffer(streamLoader.getDataForName("obj.idx"));
		totalItems = byteBuffer.readUnsignedWord();
	...
	public static Sprite getSprite(int i, int j, int k) {
		if (k == 0) {
			Sprite sprite = (Sprite) referenceCache1.insertFromCache(i);
		...
		int l3 = Texture.sin[itemDef.modelRotation1] * k3 >> 16;
		int i4 = Texture.cos[itemDef.modelRotation1] * k3 >> 16;
		...
		if (k == 0) {
			referenceCache1.removeFromCache(sprite2, i);
		}
	...
	public Model method201(int i) {
		...
		Model model = (Model) referenceCache2.insertFromCache(id);
		if (model != null) {
			return model;
		}
		...
		referenceCache2.removeFromCache(model, id);
		return model;
	}
	...
	private void readValues(ByteBuffer byteBuffer) {
		do {
			...
			if (i == 1) {
				modelID = byteBuffer.readUnsignedWord();
			} else if (i == 2) {
				name = byteBuffer.readString();
			} else if (i == 3) {
				description = byteBuffer.readBytes();
			} else if (i == 4) {
				modelZoom = byteBuffer.readUnsignedWord();
			} else if (i == 5) {
				modelRotation1 = byteBuffer.readUnsignedWord();
			} else if (i == 6) {
				modelRotation2 = byteBuffer.readUnsignedWord();
			} else if (i == 7) {
				modelOffset1 = byteBuffer.readUnsignedWord();
				if (modelOffset1 > 32767) {
					modelOffset1 -= 0x10000;
				}
			} else if (i == 8) {
				modelOffset2 = byteBuffer.readUnsignedWord();
				...
				value = byteBuffer.readDWord();
			} else if (i == 16) {
				membersObject = true;
			} else if (i == 23) {
				anInt165 = byteBuffer.readUnsignedWord();
				aByte205 = byteBuffer.readSignedByte();
			} else if (i == 24) {
				anInt188 = byteBuffer.readUnsignedWord();
			} else if (i == 25) {
				anInt200 = byteBuffer.readUnsignedWord();
				aByte154 = byteBuffer.readSignedByte();
			} else if (i == 26) {
				anInt164 = byteBuffer.readUnsignedWord();
			} else if (i >= 30 && i < 35) {
				if (groundActions == null) {
					groundActions = new String[5];
				}
				groundActions[i - 30] = byteBuffer.readString();
				...
				actions[i - 35] = byteBuffer.readString();
			} else if (i == 40) {
				int j = byteBuffer.readUnsignedByte();
				modifiedModelColors = new int[j];
				originalModelColors = new int[j];
				for (int k = 0; k < j; k++) {
					modifiedModelColors[k] = byteBuffer.readUnsignedWord();
					originalModelColors[k] = byteBuffer.readUnsignedWord();
				}

			} else if (i == 78) {
				anInt185 = byteBuffer.readUnsignedWord();
			} else if (i == 79) {
				anInt162 = byteBuffer.readUnsignedWord();
			} else if (i == 90) {
				anInt175 = byteBuffer.readUnsignedWord();
			} else if (i == 91) {
				anInt197 = byteBuffer.readUnsignedWord();
			} else if (i == 92) {
				anInt166 = byteBuffer.readUnsignedWord();
			} else if (i == 93) {
				anInt173 = byteBuffer.readUnsignedWord();
			} else if (i == 95) {
				anInt204 = byteBuffer.readUnsignedWord();
			} else if (i == 97) {
				certID = byteBuffer.readUnsignedWord();
			} else if (i == 98) {
				certTemplateID = byteBuffer.readUnsignedWord();
			} else if (i >= 100 && i < 110) {
				if (stackIDs == null) {
					stackIDs = new int[10];
					stackAmounts = new int[10];
				}
				stackIDs[i - 100] = byteBuffer.readUnsignedWord();
				stackAmounts[i - 100] = byteBuffer.readUnsignedWord();
			} else if (i == 110) {
				anInt167 = byteBuffer.readUnsignedWord();
			} else if (i == 111) {
				anInt192 = byteBuffer.readUnsignedWord();
			} else if (i == 112) {
				anInt191 = byteBuffer.readUnsignedWord();
			} else if (i == 113) {
				anInt196 = byteBuffer.readSignedByte();
			} else if (i == 114) {
				anInt184 = byteBuffer.readSignedByte() * 5;
			} else if (i == 115) {
				team = byteBuffer.readUnsignedByte();
			}
		} while (true);
	}
	...
	public int id;
	public static ReferenceCache referenceCache1 = new ReferenceCache(100);
	public static ReferenceCache referenceCache2 = new ReferenceCache(50);
